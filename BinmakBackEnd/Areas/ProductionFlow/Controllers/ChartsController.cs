    ...
    public class ChartsController : Controller
    {
        ...
        public IActionResult GenerateChart(ChartModel model)
        {
            ...
            try
            {
                ...
                ProductionFlowAsset asset = _context.ProductionFlowAssets.FirstOrDefault(id => id.AssetId == model.AssetId);
                if (asset == null)
                {
                ...
                {
                    var numQuery =
                    from num in histogramXAxis2
                    where num > histogramXAxis2[i] && num <= histogramXAxis2[i + 1]
                    select num;
                ...
                {
                    var numQuery =
                    from num in histogramXAxis3
                    where num > histogramXAxis3[i] && num <= histogramXAxis3[i + 1]
                    select num;
                ...
                {
                    var numQuery =
                    from num in histogramXAxis4
                    where num > histogramXAxis4[i] && num <= histogramXAxis4[i + 1]
                    select num;
                ...
                {
                    var numQuery =
                    from num in histogramXAxis5
                    where num > histogramXAxis5[i] && num <= histogramXAxis5[i + 1]
                    select num;
