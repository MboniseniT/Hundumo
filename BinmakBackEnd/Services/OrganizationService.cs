...
namespace BinmakBackEnd.Services
{
    ...
    public class OrganizationService
    {
        ...
        public List<OrganizationVM> GetOrganizationVMs(string reference)
        {
            ...
            {
                orginisationsVM.Add(new OrganizationVM() { Code = item.Code, RootOrganizationId = item.RootOrganizationId, DateStamp = item.DateStamp, Name = string.Format("{0} - [Organization]" ,item.Name), OrganizationId = item.OrganizationId, ParentOrganizationId = item.ParentOrganizationId, Reference = item.Reference, Height = item.Height, NodeId = item.OrganizationId, NodeType = 1 /*Type = 1 if organization*/ });
            }
            foreach (var item in productiveUnit)
            {
                orginisationsVM.Add(new OrganizationVM() { Code = item.Code, RootOrganizationId = item.RootOrganizationId, DateStamp = item.DateStamp, Name = string.Format("{0} - [Productive Unit]", item.Name), OrganizationId = item.ProductiveUnitId, ParentOrganizationId = item.ParentProductiveUnitId, Reference = item.Reference, Height = item.Height, NodeId = item.ProductiveUnitId, NodeType = 2 /*Type = 1 if productive unit*/ });
            }
            foreach (var item in equipment)
            {
                orginisationsVM.Add(new OrganizationVM() { Code = item.Code, RootOrganizationId = item.RootOrganizationId, DateStamp = item.DateStamp, Name = string.Format("{0} - [Equipment]", item.Name), OrganizationId = item.EquipmentId, ParentOrganizationId = item.ParentEquipmentId, Reference = item.Reference, Height = item.Height, NodeId = item.EquipmentId, NodeType = 3 /*Type = 1 if equipment*/ });
            }
